...
- **The mafsmith container is now `mafsmith-fastvep:0.1.0-0.4.0`** (`containers/mafsmith-fastvep_v0.1.0-0.4.0.def`, replacing `mafsmith_v0.1.0.def`). fastVEP was already baked into the old image, but installed with `cargo install --git … --branch master`, so each rebuild picked up whatever fastVEP's `master` was that day. It is now pinned to the `v0.4.0` tag and built `--locked` against fastVEP's own `Cargo.lock`. The build is two-stage, so the runtime image is `debian:bookworm-slim` plus the two binaries, `python3` and `procps` (61 MB), without the ~1.5 GB Rust toolchain. `container_mafsmith` defaults, the `test`/`local`/`citadel` profiles and the test configs all point at the new name. **Build and push the image before running**: `apptainer push mafsmith-fastvep_v0.1.0-0.4.0.sif oras://ghcr.io/crchum-citadel/mafsmith-fastvep:0.1.0-0.4.0`, and copy the `.sif` to the citadel containers directory.
